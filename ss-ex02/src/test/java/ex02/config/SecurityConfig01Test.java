...
public class SecurityConfig01Test {	
	...
	@BeforeEach //test method 하나 돌 때 마다 실행 
	public void setup(WebApplicationContext applicationContext) {
		filterChainProxy = applicationContext.getBean("springSecurityFilterChain", FilterChainProxy.class); //.getBean(Filter.class) <- filter가 하나밖에 없으면 가능
		mvc = MockMvcBuilders
	...
	public void testSecurityFilterChain01() throws Throwable{
		mvc.perform(get("/assets/images/logo.png")).andExpect(status().isOk()).andExpect(cookie().doesNotExist("MySecurityFilter01"));
	}
